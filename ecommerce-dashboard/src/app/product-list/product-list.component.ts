    ...
    this.productService.getProducts().subscribe({
      next: (data: any[]) => {
        this.products = data.map((product) => ({
          ...product,
          imageUrl: product.image ? `${environment.apiUrl}/uploads/${encodeURIComponent(product.image)}` : null,
          sliderImageUrls: product.sliderImages?.map((image: string) => `${environment.apiUrl}/uploads/${encodeURIComponent(image)}`) || [],
        }));
      },
      error: (error) => {
